...
export function useScrollToElement(
	...
	options: UseScrollToRefOptions = { triggerAutomatically: true }
) {
	const executeScroll = useCallback(() => {
		...
		// Execute the scroll
		scrollingContainer.scroll({ ...scrollPosition })
	}, [elementToScrollTo, options])

	useEffect(() => {
		if (!options.triggerAutomatically) return
		executeScroll()
	}, [executeScroll, options.triggerAutomatically]) // Trigger scroll when element or options change
